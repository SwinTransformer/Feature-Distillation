...
def build_finetune_optimizer(config, model, logger):
    ...
    elif config.MODEL.TYPE in ['vit', 'rsvit', 'vit_mae', 'vit_dino', 'vit_feat']:
        num_layers = config.MODEL.VIT.DEPTH
        get_layer_func = partial(get_vit_layer, num_layers=num_layers + 2)
    elif config.MODEL.TYPE in ['resnet']:
        layers = model.layers
        num_layers = sum(layers)
        get_layer_func = partial(get_resnet_layer, num_layers=num_layers + 2, layers=layers)
    else:
...
def get_resnet_layer(name, num_layers, layers):
    if name in ("mask_token"):
        return 0
        ...
        block_index = int(name.split('.')[1]) + sum(layers[:layer_index - 1]) + 1
        return block_index
    else:
        return num_layers - 1
