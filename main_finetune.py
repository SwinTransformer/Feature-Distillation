...
def parse_option():
    global no_wandb 
    parser = argparse.ArgumentParser('Feature Distillation fine-tuning script', add_help=False)
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, scaler):
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        if config.TRAIN.ACCUMULATION_STEPS > 1:
            ...
            optimizer.zero_grad()
            scaler.scale(loss).backward()
            if config.TRAIN.CLIP_GRAD and (config.TRAIN.OPTIMIZER.NAME.lower() != 'fusedlamb'):
                scaler.unscale_(optimizer)
                grad_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), config.TRAIN.CLIP_GRAD)
            else:
                grad_norm = get_grad_norm(model.parameters())
