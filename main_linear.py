...
from data.data_linear import build_loader
from lr_scheduler import build_scheduler
from optimizer import build_optimizer
from logger import create_logger
from utils import LARS, load_checkpoint, load_pretrained, save_checkpoint, get_grad_norm, auto_resume_helper, reduce_tensor
...
def parse_option():
    global no_wandb 
    parser = argparse.ArgumentParser('feature distillation linear probe script', add_help=False)
...
def main(config):
    ...
    # ===== add linear probe head like mae ========
    ...
    optimizer = LARS(model.head.parameters(), lr=config.TRAIN.BASE_LR, weight_decay=config.TRAIN.WEIGHT_DECAY)
    print(optimizer)
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        wandb_log(
            data=dict(acc1=acc1, acc5=acc5, val_loss=loss, max_acc=max_accuracy),
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, scaler):
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        if (epoch * num_steps + idx) % config.PRINT_FREQ == 0:
            wandb_log(
                data=dict(lr=lr, time=batch_time.val, epoch=epoch, iter=idx, loss=loss_meter.val, loss_ma=loss_meter.avg, grad_norm=norm_meter.val, loss_scale=loss_scale_meter.val),
